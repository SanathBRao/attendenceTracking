...
const CLASS_LAT = 12.9716;
const CLASS_LNG = 77.5946;
...
function Student() {
  ...
  const registerStudent = async () => {
    if (!name || !email || !address) {
      setStatus(
        "Enter name, email and connect wallet ❌"
      );
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          if (
            ...
            setStatus(
              `Outside allowed area ❌ (${Math.round(
                distance
              )}m away)`
            );
  ...
  const markAttendance = async () => {
    ...
    try {
      ...
      const tx =
        await contractRef.current.markAttendance(
          sessionId
        );
